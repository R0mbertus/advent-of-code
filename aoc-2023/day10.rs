...
fn parse(input: &str) -> (Vec<Vec<bool>>, Vec<Vec<char>>) {
    ...
    let pos_animal = (
        input.find('S').unwrap() as i64 / height,
        (input.find('S').unwrap() as i64 % width) - minus,
    );
    ...
    loop {
        ...
        current = **get_neighbours(
            maze[current.0 as usize][current.1 as usize],
            current,
            height,
            width,
        )
        .iter()
        .filter(|(y, x)| {
            get_valid_moves((y - current.0, x - current.1))
                .iter()
                .contains(&maze[*y as usize][*x as usize])
    ...
    let animal_replacement = get_neighbours('S', pos_animal, height, width)
        .into_iter()
        .filter(|(y, x)| {
            get_valid_moves((y - pos_animal.0, x - pos_animal.1))
                .iter()
                .contains(&maze[*y as usize][*x as usize])
                && visited[*y as usize][*x as usize]
        })
        .collect();
    maze[pos_animal.0 as usize][pos_animal.1 as usize] =
        moves_to_piece(pos_animal, animal_replacement);
